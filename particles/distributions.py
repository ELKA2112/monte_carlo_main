...
class DiscreteUniform(DiscreteDist):
    ...
    ----------
    lo, hi: int
        support is lo, lo + 1, ..., hi (inclusive)
    """

    def __init__(self, lo=0, hi=2):
        self.lo, self.hi = lo, hi
        self.n_values = self.hi - self.lo + 1  # number of possible values
        self.log_norm_cst = np.log(self.n_values)

    def logpdf(self, x):
        return np.where((x >= self.lo) & (x <= self.hi), -self.log_norm_cst, -np.inf)

    def rvs(self, size=None):
        return random.randint(self.lo, high=self.hi + 1, size=size)
...
class LinearD(TransformedDist):
    """Distribution of Y = AX + b where A is a matrix and b is a vector.
    ...
    a: float or ndarray
        Transformation matrix/scalar (must be invertible if matrix)
    b: float or ndarray
        Translation vector/scalar (default: 0)
    """

    def __init__(self, base_dist, a=1.0, b=0.0):
        self.a = np.atleast_2d(np.asarray(a))
        self.b = np.atleast_1d(np.asarray(b))
        self.base_dist = base_dist
        
        # Verify dimensions
        if self.a.ndim > 2:
            raise ValueError("Parameter 'a' must be a scalar or 2D matrix")
        if self.a.shape[0] != self.a.shape[1]:
            raise ValueError("Matrix 'a' must be square")
        if self.b.shape[0] != self.a.shape[0]:
            raise ValueError("Vector 'b' must have same dimension as matrix 'a'")
            
        # Cache matrix determinant for logJac
        self._logdet = np.log(np.abs(np.linalg.det(self.a)))

    def f(self, x):
        """Apply linear transformation AX + b"""
        return np.dot(self.a, x.T).T + self.b

    def finv(self, x):
        """Apply inverse transformation A^(-1)(X - b)"""
        return np.dot(np.linalg.inv(self.a), (x - self.b).T).T

    def logJac(self, x):
        """Log of absolute determinant of Jacobian matrix"""
        return self._logdet

class ScalarProduct(TransformedDist):
    """Distribution of Y = X^T X where X is a multivariate random variable.
    ...
    ----------
    base_dist: ProbDist
        The distribution of X (must be multivariate)
    """
    ...
    def f(self, x):
        # x should be of shape (N, d) where d is dimension of base_dist
        return np.sum(x * x, axis=-1)  # returns shape (N,)
        
    def finv(self, x):
        # The inverse is not uniquely defined since many vectors X can give the same
        # quadratic form value. We cannot implement this.
        raise NotImplementedError("Inverse transform is not uniquely defined for quadratic form")
        
    def logJac(self, x):
        # The Jacobian is also not well-defined due to the many-to-one nature of 
        # the transformation
        raise NotImplementedError("Jacobian is not well-defined for quadratic form")
        
    def logpdf(self, x):
        # We need to override logpdf since we can't use the default implementation
        # that relies on finv and logJac
        raise NotImplementedError("PDF computation requires specialized implementation "
                                "for quadratic forms")
...
class StructDist(ProbDist):
    ...
    def logpdf(self, theta):
        ...
        for par, law in self.laws.items():
            ...
            curr_lp = cond_law.logpdf(theta[par])
            lp += curr_lp
        return lp
